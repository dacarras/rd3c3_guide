...
We rely on this second formulation, to fit the different model specification to assess measurement invariance. In this formulation factor loadings ( $\lambda_{i}$ ) and thresholds are included ( $-\tau_{ik}$ ) per item, and a term for the theoretical factor ( $θ_{p}$ ). These terms can be used to retrieve a_i and b_ik from the IRT parameterization, using the following expressions (Wang & Wang, 2020):

$$
a_{i} = \frac{1}{\sqrt{\lambda_{i}^{-2} -1}}
